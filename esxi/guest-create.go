...
func guestCREATE(c *Config, guest_name string, disk_store string,
	...
	if vmid != "" {
		...
		stdout, _ = runRemoteSshCommand(c, remote_cmd, "check if guest path already exists.")
		...
		if strings.Contains(stdout, "No such file or directory") == true {
			...
			stdout, err = runRemoteSshCommand(c, remote_cmd, "create guest path")
			if err != nil {
		...
		vmx_contents, err = runRemoteSshCommand(c, remote_cmd, "write guest_name.vmx file")
		...
		_, err = runRemoteSshCommand(c, remote_cmd, "vmkfstools (make boot disk)")
		if err != nil {
			remote_cmd = fmt.Sprintf("rm -fr %s", fullPATH)
			stdout, _ = runRemoteSshCommand(c, remote_cmd, "cleanup guest path because of failed events")
		...
		_, err = runRemoteSshCommand(c, remote_cmd, "solo/registervm")
		if err != nil {
