...
func getPoolID(c *Config, resource_pool_name string) (string, error) {
	...
	stdout, err := runRemoteSshCommand(c, remote_cmd, "get existing resource pool id")
	if err == nil {
...
func getPoolNAME(c *Config, resource_pool_id string) (string, error) {
	...
	stdout, err := runRemoteSshCommand(c, remote_cmd, "get resource pool path")
	if err != nil {
	...
	for i := range result {
		...
		if result[i] != "path" && result[i] != "host" && result[i] != "user" && result[i] != "" {
			...
			stdout, _ := runRemoteSshCommand(c, remote_cmd, "get resource pool name")
...
func resourcePoolRead(c *Config, pool_id string) (string, int, string, int, string, int, string, int, string, error) {
	...
	stdout, err = runRemoteSshCommand(c, remote_cmd, "resource pool_config_get")
