...
func guestGetVMID(c *Config, guest_name string) (string, error) {
	...
	vmid, err = runRemoteSshCommand(c, remote_cmd, "get vmid")
...
func guestValidateVMID(c *Config, vmid string) (string, error) {
	...
	vmid, err = runRemoteSshCommand(c, remote_cmd, "validate vmid exists")
...
func getBootDiskPath(c *Config, vmid string) (string, error) {
	...
	stdout, err = runRemoteSshCommand(c, remote_cmd, "get boot disk")
	if err != nil {
...
func getDst_vmx_file(c *Config, vmid string) (string, error) {
	...
	stdout, err = runRemoteSshCommand(c, remote_cmd, "get dst_vmx")
	dst_vmx = stdout
...
func readVmx_contents(c *Config, vmid string) (string, error) {
	...
	vmx_contents, err = runRemoteSshCommand(c, remote_cmd, "read guest_name.vmx file")
...
func updateVmx_contents(c *Config, vmid string, iscreate bool, memsize int, numvcpus int,
	...
	vmx_contents, err = writeContentToRemoteFile(c, strings.Replace(vmx_contents, "\\\"", "\"", -1), dst_vmx_file, "write guest_name.vmx file")

	remote_cmd = fmt.Sprintf("vim-cmd vmsvc/reload %s", vmid)
	_, err = runRemoteSshCommand(c, remote_cmd, "vmsvc/reload")
	return err
}
...
func guestPowerOn(c *Config, vmid string) (string, error) {
	...
	stdout, err := runRemoteSshCommand(c, remote_cmd, "vmsvc/power.on")
...
func guestPowerOff(c *Config, vmid string, guest_shutdown_timeout int) (string, error) {
	...
	if savedpowerstate == "off" {
		...
		if guest_shutdown_timeout != 0 {
			...
			stdout, _ = runRemoteSshCommand(c, remote_cmd, "vmsvc/power.shutdown")
			time.Sleep(3 * time.Second)
		...
		stdout, _ = runRemoteSshCommand(c, remote_cmd, "vmsvc/power.off")
...
func guestPowerGetState(c *Config, vmid string) string {
	...
	stdout, _ := runRemoteSshCommand(c, remote_cmd, "vmsvc/power.getstate")
...
func guestGetIpAddress(c *Config, vmid string, guest_startup_timeout int) string {
	...
	for uptime < guest_startup_timeout {
		...
		stdout, _ = runRemoteSshCommand(c, remote_cmd, "get ip_address method 1")
		ip_address = stdout
		if ip_address != "" {
			return ip_address
		}
		...
		stdout, err := runRemoteSshCommand(c, remote_cmd, "get uptime")
		if err != nil {
			return ""
		}
	...
	stdout, _ = runRemoteSshCommand(c, remote_cmd, "get ip_address method 2")
	ip_address2 = stdout
	if ip_address2 != "" {
		return ip_address2
	}
