...
func resourceVIRTUALDISKDelete(d *schema.ResourceData, m interface{}) error {
	...
	stdout, err = runRemoteSshCommand(c, remote_cmd, "destroy virtual disk")
	if err != nil {
	...
	stdout, err = runRemoteSshCommand(c, remote_cmd, "Check if Storage dir is empty")
	if stdout == "3" {
		{
			...
			_, _ = runRemoteSshCommand(c, remote_cmd, "rmdir empty Storage dir")
		}
	}
